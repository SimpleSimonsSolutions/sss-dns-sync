...
class IndexController extends pm_Controller_Action
{
    ...
    public function formAction()
    {
        ...
        $form->addElement('select', 'DNS-provider', array(
            'label' => 'Select DNS Provider',
            'multiOptions' => array('opt-0' => 'IONOS', 'opt-1' => 'NotHere'),
            'value' => pm_Settings::get('DNS-provider'),
            'required' => true,
        ));
        $form->addElement('text', 'DNS-server', array(
            'label' => 'DNS Server URL',
            'value' => pm_Settings::get('DNS-server'),
            'description' => 'DNS server API URL: ' . pm_Settings::get('DNS-server'),
            'validators' => array(
                array('Uri', true, true),
            ),
        ));
        $form->addElement('text', 'DNS-API-key', array(
            'label' => 'Example TextArea',
            'value' => pm_Settings::get('DNS-API-key'),
            'required' => true,
            'validators' => array(
