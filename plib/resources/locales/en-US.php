<?php
// Copyright 1999-2017. Parallels IP Holdings GmbH.
// Modifications Copyright 2025. Simple Simon's Solutions. All rights reserved.
$messages = array(
    // Pathbar
    'leftMenuTitle' => 'DNS Sync Manager',
    'leftMenuDescription' => 'DNS Sync Manager',
    'customBackendAlert' => 'The integration of Plesk with the DNS Sync Manager extension is not enabled. <a href="%%enableUrl%%">Enable</a>',
    'customBackendEnablingError' => 'Failed to enable custom backend: %%error%%',
    'customBackendEnabled' => 'The integration of Plesk with the DNS Sync Manager extension is enabled.',

    'listPageTitle' => 'DNS Sync Manager',
    'statusColumnTitle' => 'Status',
    'configColumnTitle' => 'IP Address',
    'addToolTitle' => 'Add Slave',
    'addToolDescription' => 'Create a remote slave configuration.',
    'refreshToolTitle' => 'Refresh',
    'refreshToolDescription' => 'Refresh remote slaves status',
    'removeToolTitle' => 'Remove',
    'removeToolDescription' => 'Remove the selected slave configurations.',
    'resyncToolTitle' => 'Resync',
    'resyncToolDescription' => 'Resync all DNS zones from master to slaves.',

    'addPageTitle' => 'Creating Remote Slave Configuration',
    'howTo' => 'Add to named.conf on the remote server (config location depends on the OS of the slave server)',
    'learnMore' => 'Learn more about the slave server setup.',
    'masterIpLabel' => 'Plesk (master) IP address',
    'ipLabel' => 'Slave DNS IP address',
    'invalidIpExistingSlave' => 'This Slave DNS already exist',
    'portLabel' => 'Port',
    'algorithmLabel' => 'Algorithm',
    'secretLabel' => 'Secret',
    'invalidSecret' => 'The secret should be a base64 encoded string.',
    'viewLabel' => 'View',
    'invalidAlnumDashUnderscore' => 'The view should consists of letters, digits, dashes, underscores.',
    'slaveSaved' => 'The slave configuration was saved.',

    'viewPageTitle' => 'Remote Slave Configuration',
    'pathLabel' => 'Config path',

    'emptySelection' => 'Select items first.',
    'removeConfirmation' => 'Remove the selected configurations?',
    'resyncConfirmation' => 'Resync all DNS zones from master to slaves?',
    'confirmationYes' => 'Yes',
    'confirmationNo' => 'No',
    'resyncDone' => 'All DNS zones were synced from master to slaves.',
    'slaveRemoved' => 'The slave configurations were removed.',
);
